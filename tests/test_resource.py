...
def test_resource_call_action(resource, mocker):
    ...
    resource.api.call.assert_called_once_with(resource.api, resource.name, 'index', params, headers, {}, None, None)


def test_resource_call_action_minimal(resource, mocker):
    mocker.patch('apypie.Api.call', autospec=True)
    assert resource.call('index')
    resource.api.call.assert_called_once_with(resource.api, resource.name, 'index', {}, {}, {}, None, None)
